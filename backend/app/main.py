...
def purchase(
    ...
    with connect() as db:
        ...
        if (
            ...
            raise HTTPException(
                409,
                "Это оформление уже открыто",
            )

        # Spotlight нельзя покупать повторно,
        # пока предыдущий активен.
        if (
            body.item == "spotlight"
            and task["spotlight_until"]
            and task["spotlight_until"]
            > now()
        ):
            raise HTTPException(
                409,
                "Spotlight уже активен",
            )
